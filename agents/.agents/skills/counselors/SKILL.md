...
allowed-tools: Bash(counselors *), Bash(ps *), AskUserQuestion, Read, Write, Glob, Grep
---
...
**Timing**: Sessions commonly take more than 10 minutes. Counselors prints each child process PID alongside the agent name in its progress output (e.g. `PID 12345  claude`). If a run seems stuck, you can verify processes are still alive with `ps -p <PID>` (macOS/Linux).
