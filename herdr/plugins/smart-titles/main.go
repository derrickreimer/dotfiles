...
var (
	...
	procCache = map[string]string{}  // pane_id -> foreground process name
)
...
func foregroundProcName(paneID string) string {
	info := rpc("pane.process_info", map[string]any{"pane_id": paneID})
	if info == nil {
		return ""
	}
	pi, ok := info["process_info"].(map[string]any)
	if !ok {
		return ""
	}
	fps, ok := pi["foreground_processes"].([]any)
	if !ok || len(fps) == 0 {
		return ""
	}
...
func paneDisplayName(pane map[string]any) *string {
	...
	dataMu.Lock()
	proc := procCache[paneID]
	dataMu.Unlock()
	if proc == "" {
		proc = foregroundProcName(paneID)
		dataMu.Lock()
		procCache[paneID] = proc
		dataMu.Unlock()
	}
	if proc != "" && !shells[proc] {
		return &proc
...
// pane.updated does not include the foreground process, so opening nvim (or
// quitting back to a shell) is invisible unless we poll process_info.
func refreshProcess(paneID string) {
	proc := foregroundProcName(paneID)
	dataMu.Lock()
	prev, known := procCache[paneID]
	procCache[paneID] = proc
	...
	if getStr(pane, "agent") != "" {
		return
	}
	if known && proc == prev {
		return
	}
	logf("process %s: %q -> %q", paneID, prev, proc)
	schedulePane(paneID)
}

func watchProcesses() {
	ticker := time.NewTicker(processPoll)
	defer ticker.Stop()
	for range ticker.C {
		dataMu.Lock()
